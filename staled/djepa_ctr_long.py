...
class RobomimicVideos(Dataset):
    def __init__(self, cfg, rng, robots=['panda', 'kinova3', 'ur5e'], tasks=['can', 'lift', 'square'], stats_root = "/zfsauton/scratch/yiqiw2/robomimic_datasets_stats/robomimic_multi_new", png_root="/zfsauton/scratch/yiqiw2/robomimic_datasets", num_frames=8, img_size=128, episodes=100, ctr_shift=False):
        self.cfg = cfg
        self.rng = rng
    ...
    def __getitem__(self, idx):
        ...
        if self.ctr_shift:
            ...
            neighbors = [self.indices[idx-self.num_frames]]
            if idx+self.num_frames < len(self.indices):
                neighbors += [self.indices[idx+self.num_frames] ]
                ...
                if i % visual_interval == 0:
                    ...
                    with torch.no_grad():
                        ...
                        for frame_idx, token_slice in frame_specs:
                            ...
                            attn_map = pool(frame_tokens)[1].reshape(ctx_enc.s_grid, ctx_enc.s_grid).cpu().numpy()
