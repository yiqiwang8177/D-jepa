    ...
    for label, n_blocks, scale in MASK_GROUPS:
        ...
        if label != "ctr":
            ...
            for _ in range(B):
                ...
                pred.append(masked)
        content = {"label": label, "n_blocks": n_blocks, "block_hw": (h, w), "ctx": ctx, "pred": pred}
        if label == "ctr":
            content['i_s'] = i_s
...
def pretrain(cfg, loader, val_loader, rng, epochs, device, lr=3e-4, wd=0.05, ema_start=0.99925, ema_end=0.99925, logger = None, encoder_path=None):
    ...
    for epoch in range(epochs):
        ...
        for videos, vids in pbar:
            ...
            per = {}
            p2s = [] if (step+1) % cfg.accum_steps == 0 or step == 0 else p2s
            p2s_dyn = [] if (step+1) % cfg.accum_steps == 0 or step == 0 else p2s_dyn
